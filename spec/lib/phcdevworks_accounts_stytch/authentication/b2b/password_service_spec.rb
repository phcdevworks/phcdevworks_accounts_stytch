...
RSpec.describe PhcdevworksAccountsStytch::Authentication::B2b::PasswordService do
  ...
  let(:passwords) { instance_double(StytchB2B::Passwords) }
  let(:passwords_email) { instance_double(StytchB2B::Passwords::Email) }
  let(:passwords_existing) { instance_double(StytchB2B::Passwords::ExistingPassword) }
  let(:passwords_sessions) { instance_double(StytchB2B::Passwords::Sessions) }
  ...
  describe '#reset_start' do
    context 'when reset start is successful' do
      it 'returns a Success object' do
        ...
        result = service.reset_start(email, organization_id)
    ...
    context 'when reset start fails' do
      it 'raises a custom error' do
        ...
        expect do
          service.reset_start(email, organization_id)
  ...
  describe '#reset_existing' do
    context 'when reset existing is successful' do
      it 'returns a Success object' do
        ...
        result = service.reset_existing(email, old_password, new_password, organization_id)
    ...
    context 'when reset existing fails' do
      it 'raises a custom error' do
        ...
        expect do
          service.reset_existing(email, old_password, new_password, organization_id)
        end.to raise_error(PhcdevworksAccountsStytch::Stytch::Error,
  ...
  describe '#reset_with_session' do
    context 'when reset with session is successful' do
      it 'returns a Success object' do
        ...
        result = service.reset_with_session(session_token, new_password, organization_id)
    ...
    context 'when reset with session fails' do
      it 'raises a custom error' do
        ...
        expect do
          service.reset_with_session(session_token, new_password, organization_id)
        end.to raise_error(PhcdevworksAccountsStytch::Stytch::Error,
  ...
  describe '#authenticate_password' do
    context 'when authentication is successful' do
      it 'returns a Success object' do
        ...
        result = service.authenticate_password(email, password, organization_id)
    ...
    context 'when authentication fails' do
      it 'raises a custom error' do
        ...
        expect do
          service.authenticate_password(email, password, organization_id)
        end.to raise_error(PhcdevworksAccountsStytch::Stytch::Error,
  ...
  def allow_successful_reset(response)
    allow(passwords_email).to receive(:reset)
      .with(password_reset_token: password_reset_token, password: new_password)
      .and_return(response)
  end
  ...
  def allow_successful_reset_existing(response)
    allow(passwords_existing).to receive(:reset)
      .with(email_address: email, existing_password: old_password, new_password: new_password, organization_id: organization_id)
      .and_return(response)
  end
  ...
  def allow_successful_reset_with_session(response)
    allow(passwords_sessions).to receive(:reset)
      .with(session_token: session_token, password: new_password, organization_id: organization_id)
      .and_return(response)
  end
  ...
  def allow_successful_authenticate(response)
    allow(passwords).to receive(:authenticate)
      .with(email_address: email, password: password, organization_id: organization_id)
      .and_return(response)
  end
